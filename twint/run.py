...
import os
import sys
from asyncio import TimeoutError, ensure_future, get_event_loop
from datetime import timedelta

from . import datelock, feed, get, output, storage, verbose
    ...
    async def Feed(self):
        ...
        while True:
            ...
            if self.config.Debug:
                with open("twint-last-request.log", "a", encoding="utf-8") as f:
                    f.write(response + os.linesep)

            if self.config.Resume:
                with open(self.config.Resume, "w", encoding="utf-8") as f:
                    f.write(self.init + os.linesep)
